...
class Provider(models.Model):
    ...
    ruc = models.CharField(max_length=13, unique=True, verbose_name='Número de NIT')
...
class Category(models.Model):
    ...
    image = CustomImageField(folder='category', null=True, blank=True, verbose_name='Imagen')  
...
class Purchase(models.Model):
    ...
    end_credit = models.DateField(default=timezone.now, verbose_name='Fecha de plazo de credito')
...
class Client(models.Model):
    ...
    dni = models.CharField(max_length=13, unique=True, verbose_name='Identificación')
...
class Sale(models.Model):
    ...
    date_joined = models.DateField(default=timezone.now, verbose_name='Fecha de registro')
    end_credit = models.DateField(default=timezone.now, verbose_name='Fecha limite de credito')
    ...
    def generate_xml(self):
        ...
        xml_UBLExtension = ElementTree.SubElement(xml_UBLExtensions, 'ext:UBLExtension')
        xml_ExtensionContent = ElementTree.SubElement(xml_UBLExtension, 'ext:ExtensionContent')
        xml_DianExtensions = ElementTree.SubElement(xml_ExtensionContent, 'sts:DianExtensions')
        xml_InvoiceControl = ElementTree.SubElement(xml_DianExtensions, 'sts:InvoiceControl')

        xml_InvoiceAuthorization = ElementTree.SubElement(xml_InvoiceControl, 'sts:InvoiceAuthorization')
        xml_InvoiceAuthorization.text = '18760000001'

        # Crea el subelemento AuthorizationPeriod y sus subelementos
        xml_AuthorizationPeriod = ElementTree.SubElement(xml_InvoiceControl, 'sts:AuthorizationPeriod')
        xml_StartDate = ElementTree.SubElement(xml_AuthorizationPeriod, 'cbc:StartDate')
        xml_StartDate.text = '2019-01-19'
        xml_EndDate = ElementTree.SubElement(xml_AuthorizationPeriod, 'cbc:EndDate')
        xml_EndDate.text = '2030-01-19'

        # Crea el subelemento AuthorizedInvoices y sus subelementos
        xml_AuthorizedInvoices = ElementTree.SubElement(xml_InvoiceControl, 'sts:AuthorizedInvoices')
        xml_Prefix = ElementTree.SubElement(xml_AuthorizedInvoices, 'sts:Prefix')
        ...
        xml_InvoiceSource = ElementTree.SubElement(xml_DianExtensions, 'sts:InvoiceSource')
        xml_IdentificationCode = ElementTree.SubElement(xml_InvoiceSource, 'cbc:IdentificationCode')
        xml_IdentificationCode.set('listAgencyID', '6')
        xml_IdentificationCode.set('listAgencyName', 'United Nations Economic Commission for Europe')
        xml_IdentificationCode.set('listSchemeURI', 'urn:oasis:names:specification:ubl:codelist:gc:CountryIdentificationCode-2.1')
        xml_IdentificationCode.text = 'CO'

        # Agregar el elemento sts:SoftwareProvider
        xml_SoftwareProvider = ElementTree.SubElement(xml_DianExtensions, 'sts:SoftwareProvider')
       
        xml_ProviderID = ElementTree.SubElement(xml_SoftwareProvider, 'sts:ProviderID')
        xml_ProviderID.text = '800197268'
        xml_ProviderID.set('schemeAgencyID', '195')
        xml_ProviderID.set('schemeAgencyName', 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)')
        xml_ProviderID.set('schemeID', '4')
        xml_ProviderID.set('schemeName', '31')
        xml_SoftwareID = ElementTree.SubElement(xml_SoftwareProvider, 'sts:SoftwareID')
        xml_SoftwareID.text = '56f2ae4e-9812-4fad-9255-08fcfcd5ccb0'
        xml_SoftwareID.set('schemeAgencyID', '195')
        xml_SoftwareID.set('schemeAgencyName', 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)')
        xml_SoftwareSecurityCode = ElementTree.SubElement(xml_SoftwareProvider, 'sts:SoftwareSecurityCode')
        ...
        xml_SoftwareSecurityCode.set('schemeAgencyName', 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)')

       # Agregar la subextensión sts:SoftwareSecurityCode
        xml_SoftwareSecurityCode = ElementTree.SubElement(xml_DianExtensions, 'sts:SoftwareSecurityCode')
        xml_SoftwareSecurityCode.set('schemeAgencyID', '195')
        xml_SoftwareSecurityCode.set('schemeAgencyName', 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)')
        xml_SoftwareSecurityCode.text = 'a8d18e4e5aa00b44a0b1f9ef413ad8215116bd3ce91730d580eaed795c83b5a32fe6f0823abc71400b3d59eb542b7de8'

        # Agregar el elemento sts:AuthorizationProvider
        xml_AuthorizationProvider = ElementTree.SubElement(xml_DianExtensions, 'sts:AuthorizationProvider')

        # Agregar el subelemento de sts:AuthorizationProvider
        xml_AuthorizationProviderID = ElementTree.SubElement(xml_AuthorizationProvider, 'sts:AuthorizationProviderID')
        xml_AuthorizationProviderID.set('schemeAgencyID', '195')
        xml_AuthorizationProviderID.set('schemeAgencyName', 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)')
        ...
        xml_UBLExtension = ElementTree.SubElement(xml_UBLExtensions, 'ext:UBLExtension')
        xml_ExtensionContent = ElementTree.SubElement(xml_UBLExtension, 'ext:ExtensionContent')
        xml_Signature = ElementTree.SubElement(xml_ExtensionContent, 'ds:Signature')
        ...
        ds_CanonicalizationMethod = ElementTree.SubElement(xml_SignedInfo, 'ds:CanonicalizationMethod')
        ds_CanonicalizationMethod.set('Algorithm', 'http://www.w3.org/TR/2001/REC-xml-c14n-20010315')
        ds_SignatureMethod = ElementTree.SubElement(xml_SignedInfo, 'ds:SignatureMethod')
        ds_SignatureMethod.set('Algorithm', 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256')

        ds_Reference1 = ElementTree.SubElement(xml_SignedInfo, 'ds:Reference')
        ds_Reference1.set('Id', 'xmldsig-d0322c4f-be87-495a-95d5-9244980495f4-ref0')
        ...
        ds_Transform1.set('Algorithm', 'http://www.w3.org/2000/09/xmldsig#enveloped-signature')
        ds_DigestMethod1 = ElementTree.SubElement(ds_Reference1, 'ds:DigestMethod')
        ds_DigestMethod1.set('Algorithm', 'http://www.w3.org/2001/04/xmlenc#sha256')
        ds_DigestValue1 = ElementTree.SubElement(ds_Reference1, 'ds:DigestValue')
        ...
        ds_Reference2.set('URI', '#xmldsig-87d128b5-aa31-4f0b-8e45-3d9cfa0eec26-keyinfo')
        ds_DigestMethod2 = ElementTree.SubElement(ds_Reference2, 'ds:DigestMethod')
        ds_DigestMethod2.set('Algorithm', 'http://www.w3.org/2001/04/xmlenc#sha256')
        ds_DigestValue2 = ElementTree.SubElement(ds_Reference2, 'ds:DigestValue')
        ...
        ds_Reference3.set('URI', '#xmldsig-d0322c4f-be87-495a-95d5-9244980495f4-signedprops')
        ds_DigestMethod3 = ElementTree.SubElement(ds_Reference3, 'ds:DigestMethod')
        ds_DigestMethod3.set('Algorithm', 'http://www.w3.org/2001/04/xmlenc#sha256')
        ds_DigestValue3 = ElementTree.SubElement(ds_Reference3, 'ds:DigestValue')
        ds_DigestValue3.text = 'hpIsyD/08hVUc1exnfEyhGyKX5s3pUPbpMKmPhkPPqU='
    

        # Agregar el elemento ds:SignatureValue con su atributo Id
        ds_SignatureValue = ElementTree.SubElement(xml_Signature, 'ds:SignatureValue')
        ds_SignatureValue.set('Id', 'xmldsig-d0322c4f-be87-495a-95d5-9244980495f4-sigvalue')
        ...
        ds_KeyInfo.set('Id', 'xmldsig-87d128b5-aa31-4f0b-8e45-3d9cfa0eec26-keyinfo')
        ...
        ds_X509Certificate = ElementTree.SubElement(ds_X509Data, 'ds:X509Certificate')
        ...
        xades_QualifyingProperties = ElementTree.SubElement(ds_Object, 'xades:QualifyingProperties')
        xades_QualifyingProperties.set('Target', '#xmldsig-d0322c4f-be87-495a-95d5-9244980495f4')
        xades_SignedProperties = ElementTree.SubElement(xades_QualifyingProperties, 'xades:SignedProperties')
        xades_SignedProperties.set('Id', 'xmldsig-d0322c4f-be87-495a-95d5-9244980495f4-signedprops')
        xades_SignedSignatureProperties = ElementTree.SubElement(xades_SignedProperties, 'xades:SignedSignatureProperties')
        xades_SigningTime = ElementTree.SubElement(xades_SignedSignatureProperties, 'xades:SigningTime')
        xades_SigningTime.text = '2019-06-21T19:09:35.993-05:00'
        xades_SigningCertificate = ElementTree.SubElement(xades_SignedSignatureProperties, 'xades:SigningCertificate')
        xades_Cert = ElementTree.SubElement(xades_SigningCertificate, 'xades:Cert')
        xades_CertDigest = ElementTree.SubElement(xades_Cert, 'xades:CertDigest')
        ds_DigestMethod = ElementTree.SubElement(xades_CertDigest, 'ds:DigestMethod')
        ds_DigestMethod.set('Algorithm', 'http://www.w3.org/2001/04/xmlenc#sha256')
        ds_DigestValue = ElementTree.SubElement(xades_CertDigest, 'ds:DigestValue')
        ds_DigestValue.text = 'nem6KXhqlV0A0FK5o+MwJZ3Y1aHgmL1hDs/RMJu7HYw='
        xades_SignaturePolicyIdentifier = ElementTree.SubElement(xades_SignedSignatureProperties, 'xades:SignaturePolicyIdentifier')
        xades_SigPolicyId = ElementTree.SubElement(xades_SignaturePolicyIdentifier, 'xades:SignaturePolicyId')
        xades_Identifier = ElementTree.SubElement(xades_SigPolicyId, 'xades:Identifier')
        xades_Identifier.text = 'https://facturaelectronica.dian.gov.co/politicadefirma/v1/politicadefirmav2.pdf'
        xades_SigPolicyHash = ElementTree.SubElement(xades_SigPolicyId, 'xades:SigPolicyHash')
        ds_DigestMethod2 = ElementTree.SubElement(xades_SigPolicyHash, 'ds:DigestMethod')
        ds_DigestMethod2.set('Algorithm', 'http://www.w3.org/2001/04/xmlenc#sha256')
        ds_DigestValue2 = ElementTree.SubElement(xades_SigPolicyHash, 'ds:DigestValue')
        ds_DigestValue2.text = 'dMoMvtcG5aIzgYo0tIsSQeVJBDnUnfSOfBpxXrmor0Y='
        xades_SignerRole = ElementTree.SubElement(xades_SignedSignatureProperties, 'xades:SignerRole')
        xades_ClaimedRoles = ElementTree.SubElement(xades_SignerRole, 'xades:ClaimedRoles')
        xades_ClaimedRole = ElementTree.SubElement(xades_ClaimedRoles, 'xades:ClaimedRole')
        ...
        xml_CustomizationID = ElementTree.SubElement(root, 'cbc:CustomizationID')
        xml_CustomizationID.text = '10'

        xml_ProfileID = ElementTree.SubElement(root, 'cbc:ProfileID')
        xml_ProfileID.text = 'DIAN 2.1'

        xml_ProfileExecutionID = ElementTree.SubElement(root, 'cbc:ProfileExecutionID')
        ...
        xml_InvoiceTypeCode = ElementTree.SubElement(root, 'cbc:InvoiceTypeCode')
        xml_InvoiceTypeCode.text = '01'
        
        xml_DocumentCurrencyCode = ElementTree.SubElement(root, 'cbc:DocumentCurrencyCode')
        xml_DocumentCurrencyCode.text = 'COP'
        xml_DocumentCurrencyCode.set('listAgencyID', '6')
        xml_DocumentCurrencyCode.set('listAgencyName', 'United Nations Economic Commission for Europe')
        xml_DocumentCurrencyCode.set('listID', 'ISO 4217 Alpha')

        # Numero de productos en la fctura
        xml_LineCountNumeric = ElementTree.SubElement(root, 'cbc:LineCountNumeric')
        xml_LineCountNumeric.text = '2' 

        xml_InvoicePeriod = ElementTree.SubElement(root, 'cac:InvoicePeriod')
        xml_StartDate = ElementTree.SubElement(xml_InvoicePeriod, 'cbc:StartDate')
        ...
        cac_AccountingSupplierParty = ElementTree.SubElement(root, 'cac:AccountingSupplierParty')

        cbc_AdditionalAccountID = ElementTree.SubElement(cac_AccountingSupplierParty, 'cbc:AdditionalAccountID')
        cbc_AdditionalAccountID.text = '1'
        cac_Party = ElementTree.SubElement(cac_AccountingSupplierParty, 'cac:Party')
        cbc_IndustryClasificationCode = ElementTree.SubElement(cac_Party, 'cbc:IndustryClasificationCode')
        ...
        cac_PhysicalLocation = ElementTree.SubElement(cac_Party, 'cac:PhysicalLocation')
        cac_Address = ElementTree.SubElement(cac_PhysicalLocation, 'cac:Address')
        ...
        cbc_CountrySubentity = ElementTree.SubElement(cac_Address, 'cbc:CountrySubentity')
        cbc_CountrySubentity.text = 'Bogotá'
        cbc_CountrySubentityCode = ElementTree.SubElement(cac_Address, 'cbc:CountrySubentityCode')
        cbc_CountrySubentityCode.text = '11'
        cac_AddressLine = ElementTree.SubElement(cac_Address, 'cac:AddressLine')
        ...
        cbc_IdentificationCode = ElementTree.SubElement(cac_Country, 'cbc:IdentificationCode')
        cbc_IdentificationCode.text = 'CO'
        cbc_Name = ElementTree.SubElement(cac_Country, 'cbc:Name', languageID="es")
        cbc_Name.text = 'Colombia'

        # Agregar subelemento cac:PartyTaxScheme
        cac_PartyTaxScheme = ElementTree.SubElement(cac_Party, 'cac:PartyTaxScheme')
        cbc_RegistrationName = ElementTree.SubElement(cac_PartyTaxScheme, 'cbc:RegistrationName')
        cbc_RegistrationName.text = 'DIAN'
        cbc_CompanyID = ElementTree.SubElement(cac_PartyTaxScheme, 'cbc:CompanyID', schemeAgencyID="195", schemeAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)", schemeID="4", schemeName="31")
        cbc_CompanyID.text = '800197268'
        cbc_TaxLevelCode = ElementTree.SubElement(cac_PartyTaxScheme, 'cbc:TaxLevelCode', listName="05")
        cbc_TaxLevelCode.text = 'O-99'
        cac_RegistrationAddress = ElementTree.SubElement(cac_PartyTaxScheme, 'cac:RegistrationAddress')
        cbc_ID = ElementTree.SubElement(cac_RegistrationAddress, 'cbc:ID')
        cbc_ID.text = '11001'
        cbc_CityName = ElementTree.SubElement(cac_RegistrationAddress, 'cbc:CityName')
        cbc_CityName.text = 'Bogotá, D.C.'
        cbc_CountrySubentity = ElementTree.SubElement(cac_RegistrationAddress, 'cbc:CountrySubentity')
        cbc_CountrySubentity.text = 'Bogotá'
        cbc_CountrySubentityCode = ElementTree.SubElement(cac_RegistrationAddress, 'cbc:CountrySubentityCode')
        cbc_CountrySubentityCode.text = '11'
        cac_AddressLine = ElementTree.SubElement(cac_RegistrationAddress, 'cac:AddressLine')
        cbc_Line = ElementTree.SubElement(cac_AddressLine, 'cbc:Line')
        cbc_Line.text = 'Av. Jiménez #7 - 13'
        cac_Country = ElementTree.SubElement(cac_RegistrationAddress, 'cac:Country')
        cbc_IdentificationCode = ElementTree.SubElement(cac_Country, 'cbc:IdentificationCode')
        cbc_IdentificationCode.text = 'CO'
        cbc_Name = ElementTree.SubElement(cac_Country, 'cbc:Name', languageID="es")
        cbc_Name.text = 'Colombia'
        cac_TaxScheme = ElementTree.SubElement(cac_PartyTaxScheme, 'cac:TaxScheme')
        ...
        cac_PartyLegalEntity = ElementTree.SubElement(cac_Party, 'cac:PartyLegalEntity')
        cbc_RegistrationName = ElementTree.SubElement(cac_PartyLegalEntity, 'cbc:RegistrationName')
        cbc_RegistrationName.text = 'DIAN'
        cbc_CompanyID = ElementTree.SubElement(cac_PartyLegalEntity, 'cbc:CompanyID', schemeAgencyID="195", schemeAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)", schemeID="9", schemeName="31")
        cbc_CompanyID.text = '800197268'

        cac_AccountingCustomerParty = ElementTree.SubElement(root, 'cac:AccountingCustomerParty')
        cbc_AdditionalAccountID = ElementTree.SubElement(cac_AccountingCustomerParty, 'cbc:AdditionalAccountID')
        cbc_AdditionalAccountID.text = '1'
        cac_Party = ElementTree.SubElement(cac_AccountingCustomerParty, 'cac:Party')
        cac_PartyName = ElementTree.SubElement(cac_Party, 'cac:PartyName')
        cbc_Name = ElementTree.SubElement(cac_PartyName, 'cbc:Name')
        ...
        cbc_CompanyID = ElementTree.SubElement(cac_PartyTaxScheme, 'cbc:CompanyID', schemeAgencyID="195", schemeAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)", schemeID="3", schemeName="31")
        cbc_CompanyID.text = '900108281'
        cbc_TaxLevelCode = ElementTree.SubElement(cac_PartyTaxScheme, 'cbc:TaxLevelCode', listName="04")
        cbc_TaxLevelCode.text = 'O-99'
        cac_RegistrationAddress = ElementTree.SubElement(cac_PartyTaxScheme, 'cac:RegistrationAddress')
        cbc_ID = ElementTree.SubElement(cac_RegistrationAddress, 'cbc:ID')
        cbc_ID.text = '11001'
        ...
        cbc_RegistrationName = ElementTree.SubElement(cac_PartyLegalEntity, 'cbc:RegistrationName')
        cbc_RegistrationName.text = 'OPTICAS GMO COLOMBIA S A S'
        cbc_CompanyID = ElementTree.SubElement(cac_PartyLegalEntity, 'cbc:CompanyID', schemeAgencyID="195", schemeAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)", schemeID="3", schemeName="31")
        cbc_CompanyID.text = '900108281'
        ...
        for tax_percent in taxes:
            if 10 != 0.0:          
                ...
                cbc_TaxAmount = ElementTree.SubElement(cac_TaxTotal, 'cbc:TaxAmount', currencyID="COP")
                cbc_TaxAmount.text = str(1)
              
                cac_TaxCategory = ElementTree.SubElement(cac_TaxTotal, 'cac:TaxCategory')
                cbc_Percent = ElementTree.SubElement(cac_TaxCategory, 'cbc:Percent')
                cbc_Percent.text = '10'
                cac_TaxScheme = ElementTree.SubElement(cac_TaxCategory, 'cac:TaxScheme')
                cbc_ID = ElementTree.SubElement(cac_TaxScheme, 'cbc:ID')
                cbc_ID.text = '1'
                cbc_Name = ElementTree.SubElement(cac_TaxScheme, 'cbc:Name')
                cbc_Name.text = '1'
        ...
        cac_LegalMonetaryTotal = ElementTree.SubElement(root, 'cac:LegalMonetaryTotal')
        cbc_LineExtensionAmount = ElementTree.SubElement(cac_LegalMonetaryTotal, 'cbc:LineExtensionAmount', currencyID="COP")
        cbc_LineExtensionAmount.text = "12600.06"
        cbc_TaxExclusiveAmount = ElementTree.SubElement(cac_LegalMonetaryTotal, 'cbc:TaxExclusiveAmount', currencyID="COP")
        cbc_TaxExclusiveAmount.text = "12787.56"
        cbc_TaxInclusiveAmount = ElementTree.SubElement(cac_LegalMonetaryTotal, 'cbc:TaxInclusiveAmount', currencyID="COP")
        cbc_TaxInclusiveAmount.text = "15024.07"
        cbc_AllowanceTotalAmount = ElementTree.SubElement(cac_LegalMonetaryTotal, 'cbc:AllowanceTotalAmount', currencyID="COP")
        cbc_AllowanceTotalAmount.text = "0.00"
        cbc_PrepaidAmount = ElementTree.SubElement(cac_LegalMonetaryTotal, 'cbc:PrepaidAmount', currencyID="COP")
        cbc_PrepaidAmount.text = "0.00"
        cbc_PayableAmount = ElementTree.SubElement(cac_LegalMonetaryTotal, 'cbc:PayableAmount', currencyID="COP")
        ...
        for producto in taxes:           
            ...
            cbc_ID.text = "1"
            cbc_InvoicedQuantity = ElementTree.SubElement(cac_InvoiceLine, 'cbc:InvoicedQuantity', unitCode="EA")
            cbc_InvoicedQuantity.text = "1.000000"
            cbc_LineExtensionAmount = ElementTree.SubElement(cac_InvoiceLine, 'cbc:LineExtensionAmount', currencyID="COP")
            cbc_LineExtensionAmount.text = "12600.06"
            cbc_FreeOfChargeIndicator = ElementTree.SubElement(cac_InvoiceLine, 'cbc:FreeOfChargeIndicator')
            cbc_FreeOfChargeIndicator.text = "false"

            cac_AllowanceCharge = ElementTree.SubElement(cac_InvoiceLine, 'cac:AllowanceCharge')
            cbc_ID = ElementTree.SubElement(cac_AllowanceCharge, 'cbc:ID')
            cbc_ID.text = "1"
            cbc_ChargeIndicator = ElementTree.SubElement(cac_AllowanceCharge, 'cbc:ChargeIndicator')
            cbc_ChargeIndicator.text = "false"
            cbc_AllowanceChargeReason = ElementTree.SubElement(cac_AllowanceCharge, 'cbc:AllowanceChargeReason')
            cbc_AllowanceChargeReason.text = "Descuento por cliente frecuente"
            cbc_MultiplierFactorNumeric = ElementTree.SubElement(cac_AllowanceCharge, 'cbc:MultiplierFactorNumeric')
            cbc_MultiplierFactorNumeric.text = "33.33"
            cbc_Amount = ElementTree.SubElement(cac_AllowanceCharge, 'cbc:Amount', currencyID="COP")
            cbc_Amount.text = "6299.94"
            cbc_BaseAmount = ElementTree.SubElement(cac_AllowanceCharge, 'cbc:BaseAmount', currencyID="COP")
            cbc_BaseAmount.text = "18900.00"

            cac_TaxTotal = ElementTree.SubElement(cac_InvoiceLine, 'cac:TaxTotal')
            cbc_TaxAmount = ElementTree.SubElement(cac_TaxTotal, 'cbc:TaxAmount', currencyID="COP")
            cbc_TaxAmount.text = "2394.01"
            cac_TaxSubtotal = ElementTree.SubElement(cac_TaxTotal, 'cac:TaxSubtotal')
            cbc_TaxableAmount = ElementTree.SubElement(cac_TaxSubtotal, 'cbc:TaxableAmount', currencyID="COP")
            cbc_TaxableAmount.text = "12600.06"
            cbc_TaxAmount = ElementTree.SubElement(cac_TaxSubtotal, 'cbc:TaxAmount', currencyID="COP")
            cbc_TaxAmount.text = "2394.01"
            cac_TaxCategory = ElementTree.SubElement(cac_TaxSubtotal, 'cac:TaxCategory')
            cbc_Percent = ElementTree.SubElement(cac_TaxCategory, 'cbc:Percent')
            ...
            cbc_Description = ElementTree.SubElement(cac_Item, 'cbc:Description')
            cbc_Description.text = "AV OASYS -2.25 (8.4) LENTE DE CONTATO"
            cac_SellersItemIdentification = ElementTree.SubElement(cac_Item, 'cac:SellersItemIdentification')
            cbc_ID = ElementTree.SubElement(cac_SellersItemIdentification, 'cbc:ID')
            cbc_ID.text = "AOHV84-225"
            cac_AdditionalItemIdentification = ElementTree.SubElement(cac_Item, 'cac:AdditionalItemIdentification')
            cbc_ID = ElementTree.SubElement(cac_AdditionalItemIdentification, 'cbc:ID', schemeID="999", schemeName="EAN13")
            cbc_ID.text = "6543542313534"

            cac_Price = ElementTree.SubElement(cac_InvoiceLine, 'cac:Price')
            cbc_PriceAmount = ElementTree.SubElement(cac_Price, 'cbc:PriceAmount', currencyID="COP")
            cbc_PriceAmount.text = "18900.00"
            cbc_BaseQuantity = ElementTree.SubElement(cac_Price, 'cbc:BaseQuantity', unitCode="EA")
